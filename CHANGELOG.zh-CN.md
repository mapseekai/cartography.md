...
- 加固 YAML 表示层检查、Markdown 章节边界、Token 循环检测、语义 diff 路径和原型安全查找。
- 保留 ArcGIS CIM 颜色 Alpha、跨瓦片字段缺失证据，并修正文档中的初始化器 CLI 调用方式。

## 0.4.0 - 2026-09-07

### Breaking

- 包版本和格式版本均为 0.4.0；消费者必须使用新视觉语义，0.3.0 文档需语义审查，不自动升级。

### Changed

- 明确主体尺寸、每侧套线、向外轮廓与光晕、描边宽度、颜色优先级、间距/偏移语境及透明度合成。
- 明确稳定基础表达、语义尺度替换和不变量、角色/状态职责、显式组合状态与冲突优先级。
- 增加专题表达、编写与迁移指南、适配契约及技术网络/统计专题示例；修订 Quiet Atlas 与 Agent 决策顺序。

### Tooling

- 修正初始化器分组、半径到直径换算、保守角色/状态处理、完整 Typography 比较和扩展保留。
- 区分相同值与相同设计语义，报告共享候选；来源、统计和尺度事实移入报告，默认九章并保留缺少证据的 TODO。
- 同步 Schema、API、版本诊断与回归测试。
