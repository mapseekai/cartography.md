...
## 3. 实证：当前 schema 的真实约束

对 `schema/cartography.ts` 的 zod 定义做运行时验证（仅写 `version/name/target/intent` 的文档）：
...
即：**即使值可以为空容器，`data`、`zoom`、`tokens`、`encodings` 四个键也必须存在，且 `layerOrder` 必须至少 1 项。** 因此"只写意图"的文档今天无法通过 lint。
...
| 五个脚手架键 + 非空 `layerOrder` | **必填**（这是"必须要有数据基础"观感的真正来源） |
...
1. `schema/cartography.ts`：五个键转可选 / 条件必填（zod `superRefine` 处理 layerOrder 条件）；
2. `schema/*.schema.json`：两份 JSON Schema 同步（`required` 收敛 + `allOf` 条件）；
3. 规则守卫：`cartography.ts` 中 `zoom.bands`、`tokens.colors`、`layerOrder`、`encodings` 的直接访问改为条件化（现有 `!context.cartography` 早退不够用）；
4. 测试：新增 L0/L1 通过用例 + L2 缺画像用例（注意：现有 `example.test.ts` 第二用例断言 `dataProfileChecked === true`，因契约声明了 `data.profile` 且 `lintFile` 自动加载——不要误引为"无画像可用"的证据）；
5. spec：新增分级章节（en/zh-CN）、必填集表格修订。
