...
describe('diff and specification', () => {
  it('detects 0.4.0 token values and sections that changed', () => {
  ...
  it('ships non-empty bilingual 0.4.0 specifications', async () => {
    const [english, chinese] = await Promise.all([readFile('../../docs/spec.md', 'utf8'), readFile('../../docs/spec.zh-CN.md', 'utf8')]);
    expect(english).toContain('0.4.0');
    expect(chinese).toContain('0.4.0');
  });
});
